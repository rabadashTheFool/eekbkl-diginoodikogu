<script setup lang="ts">
  ...
  import { humanize, lastRightPart } from '@servicestack/client'
  ...
  const sectionId = computed(() =>
    lastRightPart(useRouter().currentRoute.value.path, '/')
  )
  const activeSection = computed(
    () =>
      (sectionId.value ? sections[sectionId.value] : null) ?? sections.dashboard
  )
</script>
  ...
  <div id="app" v-cloak>
    <SidebarLayout ref="sidebar">
      <div class="flex h-16 shrink-0 items-center">
        <RouterLink
          ...
          <Logo
            class="mr-1 h-8 w-auto text-indigo-600 dark:text-indigo-300"
            alt="Logo" />
        ...
        <ul role="list" class="flex flex-1 flex-col gap-y-7">
          <li
            v-for="group in new Set(
              Object.keys(sections).flatMap((k) => sections[k].group)
            )">
            <div
              v-if="group"
            ...
            <ul role="list" class="-mx-2 space-y-1">
              <li
                v-for="section in Object.keys(sections)
                  .map((k) =>
                    sections[k].group === group ? sections[k] : null
                  )
                  .filter((x) => !!x)">
                <RouterLink
                  :to="`/admin/${section.page ?? section.id}`"
                  :class="[
                    activeSection.id === section.id
                      ? 'bg-gray-50 dark:bg-gray-900 text-indigo-600 dark:text-indigo-300'
                      : 'cursor-pointer text-gray-700 dark:text-gray-200 hover:text-indigo-600 dark:hover:text-indigo-300 hover:bg-gray-50 dark:hover:bg-gray-900',
                    'group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold select-none',
                  ]">
                  <Icon
                    :svg="section.icon"
                    class="h-6 w-6 shrink-0 text-indigo-600 dark:text-indigo-300" />
                  {{ section.label }}
                </RouterLink>
              </li>
            </ul>
      ...
      <template #mobiletitlebar>
        <div
          class="flex-1 text-sm font-semibold leading-6 text-gray-900 dark:text-gray-50">
          {{ activeSection.title }}
        </div>
        <span v-if="user" class="cursor-pointer bg-gray-50 dark:bg-gray-900">
          <span class="sr-only">Your profile</span>
          <img
            class="h-8 w-8 rounded-full bg-gray-50 dark:bg-gray-900"
            :src="user.profileUrl"
            alt="" />
        </span>
      </template>
    ...
    <main class="lg:pl-72">
      <div class="px-4 sm:px-6 lg:px-8">
        ...
        <div v-else>
          <h1
            class="hidden lg:block pt-4 mb-2 text-3xl font-bold leading-tight tracking-tight text-gray-900 dark:text-gray-50">
            {{ activeSection.title }}
          </h1>
          <RouterView />
        </div>
