...
export class ExtendedTransposeCalculator
  implements IExtendedTransposeCalculator
{
  ...
  public transposePitch(
    ...
    if (!this.Options.TransposeByDiatonic) {
      if (this.Options.KeySignaturesIsRemoved) {
        ...
        const degree: number = this.pitchToDegree(
          pitch,
        ...
        if (
          this.Options.TransposeByKey ||
          this.Options.TransposeByKeyRelation
        ) {
          // TRANSPOSE BY KEY WITHOUT KEY SIGNATURE TRANSPOSING
          const degreeOfKey: number = this.pitchToDegree(
            pitch,
            currentKeyInstruction.keyTypeOriginal
          )
          return this.commaToPitch(
            degreeOfKey + currentKeyInstruction.isTransposedBy
          )
      ...
      const srcFundamentalDegree: number = ETC.FundamentalNotes.indexOf(
        pitch.FundamentalNote
      )
      const dstFundamentalNote: number =
        ETC.FundamentalNotes[
          (srcFundamentalDegree +
            (currentKeyInstruction.isTransposedBy % 7) +
            7) %
            7
        ]
      const octaveShift: number = Math.floor(
        (srcFundamentalDegree + currentKeyInstruction.isTransposedBy) / 7
      )
      const modulatedDiatonicDegree: number = ETC.FundamentalCommas.indexOf(
        ((degree % ETC.OctaveSize) + ETC.OctaveSize) % ETC.OctaveSize
      )
      if (modulatedDiatonicDegree >= 0) {
        const dstFundamentalComma: number =
          ETC.FundamentalCommas[
            (modulatedDiatonicDegree +
              (currentKeyInstruction.isTransposedBy % 7) +
              7) %
              7
          ]
        const etcPitch: ETCPitch = ETC.commaToPitch(
          ETC.keyToComma(currentKeyInstruction.Key) + dstFundamentalComma
        )
        return new Pitch(
  ...
  public transposeKey(keyInstruction: KeyInstruction, transpose: number): void {
    ...
    if (this.Options.TransposeByKey || this.Options.TransposeByKeyRelation) {
      ...
      keyInstruction.Key = ETC.keyToMajorKey(
        transposeToKey - this.MainKey + keyInstruction.keyTypeOriginal
      )

      keyInstruction.isTransposedBy =
        ETC.keyToKeyProximity(
          keyInstruction.keyTypeOriginal,
          keyInstruction.Key
        )[this.Options.TransposeDirection] +
        this.Options.TransposeOctave * ETC.OctaveSize

      if (!this.Options.KeySignaturesIsTransposed) {
        keyInstruction.Key = keyInstruction.keyTypeOriginal
      }
    } else if (
      this.Options.KeySignaturesIsTransposed &&
      this.Options.TransposeByInterval
    ) {
      const octave: number = Math.floor(transpose / 12)
      const semitone: number = ((transpose % 12) + 12) % 12
      if (transpose % 12 !== 0) {
        keyInstruction.Key = ETC.keyToMajorKey(
          keyInstruction.keyTypeOriginal + semitone * ETC.KeyDiatonicFactor
        )
      } else {
        keyInstruction.Key = keyInstruction.keyTypeOriginal
      }
      keyInstruction.isTransposedBy =
        ...
        octave * ETC.OctaveSize
    } else if (
      this.Options.KeySignaturesIsTransposed &&
      this.Options.TransposeByHalftone
    ) {
      const octave: number = Math.floor(transpose / 12)
      ...
      if (transpose !== 0) {
        if (keyInstruction.keyTypeOriginal >= 0) {
          keyInstruction.Key = ETC.keyToSimplifiedMajorKey(
            keyInstruction.keyTypeOriginal + semitone * ETC.KeyDiatonicFactor
          )
        } else {
          keyInstruction.Key = ETC.keyToSimplifiedMajorKey(
            keyInstruction.keyTypeOriginal + semitone * ETC.KeyChromaticFactor
          )
        }
      } else {
        keyInstruction.Key = keyInstruction.keyTypeOriginal
