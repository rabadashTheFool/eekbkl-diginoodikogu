<script setup lang="ts">
  ...
  client.swr(
    new AdminData(),
    (r: ApiResult<AdminDataResponse>) =>
      (stats.value = r.response?.pageStats || [])
  )
  ...
  <div>
    <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
      <RouterLink
        v-for="stat in stats"
        :key="stat.label"
        :to="`/admin/${stat.label.toLowerCase()}`"
        ...
        <dt class="truncate text-sm font-medium text-gray-500">
          Total {{ humanize(stat.label) }}
        </dt>
        <dd
          class="mt-1 text-3xl font-semibold tracking-tight text-gray-900 dark:text-gray-50">
    ...
    <p class="mt-8 text-sm font-semibold leading-6">
      Go to
      <RouterLink to="/" class="text-indigo-600"
        >Home <span aria-hidden="true">&rarr;</span></RouterLink
      >
    </p>
    <div>
      <div class="mt-20 mx-auto text-gray-500 max-w-screen-lg">
        ...
        <p
          class="my-3 mx-auto text-base text-gray-500 sm:text-lg md:mt-5 md:text-xl">
          In this video we explore the Server Multi Razor Page and Client
          rendered Admin UI Pages to see how to use the new SidebarLayout and
          <a href="https://docs.servicestack.net/vue/autoquerygrid"
            >AutoQueryGrid</a
          >
          Vue Tailwind components to build beautiful Admin UI Pages within
          minutes.
        </p>
        <iframe
          class="mt-4 w-full aspect-video"
          src="https://www.youtube.com/embed/wlRA4_owEsc"
          frameborder="0"
