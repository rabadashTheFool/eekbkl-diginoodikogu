    ...
    <div class="shadow sm:overflow-hidden sm:rounded-md">
      <div class="space-y-6 py-6 px-4 sm:p-6 bg-white dark:bg-black">
        ...
        <fieldset>
          ...
          <div>
            <div class="mb-4">
              <TextInput id="nimi" v-model="request.nimetus" required placeholder="Variatsiooni nimi" />
            </div>
            <div class="mb-4">
      ...
      <div class="mt-4 bg-gray-50 dark:bg-gray-800 px-4 py-3 text-right sm:px-12">
        <div class="flex justify-between space-x-3">
          <div>
            ...
            <ConfirmDelete @delete="onDelete" color="red">Kustuta</ConfirmDelete>
          </div>
          <div>
...
import { UpdateVariatsioon, Variatsioon, DeleteVariatsioon } from '@/dtos'
import { computed, ref, onMounted } from 'vue'

const props = defineProps<{
  variatsioon: Variatsioon
}>()
...
const request = ref(new UpdateVariatsioon())
...
onMounted(async () => {
  request.value.id = props.variatsioon.id
  request.value.nimetus = props.variatsioon.nimetus
  request.value.helistik = props.variatsioon.helistik
  request.value.chordPro = props.variatsioon.chordPro
  request.value.musicXml = props.variatsioon.musicXml
})
...
const onDelete = async () => {
  const api = await client.apiVoid(new DeleteVariatsioon({ id: props.variatsioon.id}))
  if (api.succeeded) {
    emit("done", false)
  }
}
