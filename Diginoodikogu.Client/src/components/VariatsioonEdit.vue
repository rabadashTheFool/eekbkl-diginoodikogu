<script setup lang="ts">
  import { DeleteVariatsioon, UpdateVariatsioon, Variatsioon } from '@/dtos'
  ...
  const onDelete = async () => {
    const api = await client.apiVoid(
      new DeleteVariatsioon({ id: props.variatsioon.id })
    )
    if (api.succeeded) {
      emit('done', false)
    }
      ...
      <div
        ...
        <div class="flex justify-between space-x-3">
          <div>
            ...
            >
            <ConfirmDelete @delete="onDelete" color="red"
              >Kustuta</ConfirmDelete
            >
          </div>
          <div>
